...
map('world','South Korea', fill=T, col="gray", xlim=c(125,131))
...
tables[,3]<-paste(tables[,3])

tables[tables[,3]=="20 이상",3]=20 
...
head(real.data)
real.data$speed <-as.numeric(paste(real.data$speed))
...
pred.level<-cut(link.gis$speed,cuts,rainbow(n = 24, start=0, end=2/6)) #속도에 따른 속도 부여
table(pred.level)
cuts
